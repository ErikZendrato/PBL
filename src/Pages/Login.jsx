...
function Login() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Kirim data user DAN token ke context
        ...
        <div className="auth-form-side">
          <form className="auth-form" onSubmit={handleSubmit}>
            ...
            <div className="auth-input-group" style={{ position: "relative" }}>
              <label>Password</label>
              <input type={showPassword ? "text" : "password"} className="auth-input" value={password} onChange={(e) => setPassword(e.target.value)} required />
              <span onClick={() => setShowPassword(!showPassword)} style={{ position: "absolute", right: "12px", top: "44px", cursor: "pointer" }}>
                {showPassword ? <FiEyeOff /> : <FiEye />}
              </span>
            </div>
            <button type="submit" className="auth-button-primary blue">Login</button>
            <div className="auth-link-text">Lupa Password? <Link to="/reset-password">Klik di sini</Link></div>
            <div className="auth-link-text">
              <Link to="/register" className="auth-link-button green">Create new account</Link>
            </div>
          </form>
        </div>
      </div>
