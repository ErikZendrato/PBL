...
//   // Fungsi navigasi yang mengirim data ke Invoice
//   const handleLanjutBayar = () => {
//     const sNames = services.filter(s => servicesSelected.includes(s.id)).map(s => s.name);
//     const eNames = equipments.filter(e => equipmentsSelected.includes(e.id)).map(e => e.name);
//     const rincianAlat = [...sNames, ...eNames].join(", ") || "-";
  ...
  return (
    <div className="booking-modern">
      ...
      <section className="summary-card">
        <h2>Total Pembayaran</h2>
        <p className="price">{formatCurrency(total)}</p>
        <button className="btn-primary" disabled={!date} onClick={handleLanjutBayar}>
          Lanjut Bayar
        </button>
      </section>
    </div>
